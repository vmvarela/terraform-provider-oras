# terraform-provider-oras — operational reference

Module: `github.com/vmvarela/terraform-provider-oras`.

Ground truth extracted from `AGENTS.md` at HEAD (pre-rewrite). Facts only.
...
- Lint config: `.golangci.yml` (v2 format). gosec G304/G204 excluded repo-wide (reading user-configured CA/Docker config paths and exec'ing credential helpers are core features); G118 excluded in `client.go` (retention goroutine deliberately outlives the request context)
